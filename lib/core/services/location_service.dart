...
class LocationService {
  ...
  Future<goa.Placemark?> getPlaceMark({required LocationData locationData}) async {
    ...
    Future<goa.Placemark?> getPlaceMarkSet({required double latData, required double lonData}) async {
    final List<goa.Placemark>? placeMarks = await goa.placemarkFromCoordinates(latData, lonData);
    if(placeMarks != null && placeMarks.isNotEmpty){
      return placeMarks[0];
    }
